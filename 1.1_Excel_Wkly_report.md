...
- INDEX与MATCH嵌套使用（注意锁住的行、列）：
  ...
  2. 查找并求和 （INDEX() 若给出横坐标0，则会计入所有行 即 求一整列的和；纵坐标0类似求一整行的和）
  e.g. =SUMIFS(__INDEX(__'拌客源数据1-8月'!$1:$1048576,__0__,MATCH('常用函数-练习版'!G$111,'拌客源数据1-8月'!$1:$1,0)__)__,'拌客源数据1-8月'!$I:$I,'常用函数-练习版'!$B112), where filtering the row is conducted in SUMIFS but not in INDEX, since "0" in INDEX completes summation instead
